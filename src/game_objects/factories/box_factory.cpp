           ...
           float mass)
{
  ...
  if (mass <= 0) {
    particle = std::make_shared<Particle>(box_transform);
  } else {
    particle = std::make_shared<Particle>(box_transform, (1 / mass));
  }
  std::unique_ptr<BoxCollider> collider =
    create_box_collider(width, height, box_transform, particle);
