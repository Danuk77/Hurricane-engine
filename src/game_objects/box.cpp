...
void Box::load_shaders(){
  std::string shader_root_path = std::string(PROJECT_ROOT) + "/src/shaders/programs/";
  std::string vertex_shader = read_shader_file((shader_root_path + "/vertex_shader.glsl").c_str());
  std::string fragment_shader = read_shader_file((shader_root_path + "/fragment_shader.glsl").c_str());
...
void Box::initialise_model_matrix() {
  ...
  model_matrix = glm::translate(model_matrix, glm::vec3(0.5f * transform.size.x,
