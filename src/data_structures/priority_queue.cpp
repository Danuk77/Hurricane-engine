...
template <typename T>
void PriorityQueue<T>::insert(std::unique_ptr<T> item_to_insert) {
  items.push_back(std::move(item_to_insert));
  _bubble_up();
};

template <typename T> std::unique_ptr<T> PriorityQueue<T>::pop() {
  if (items.empty()) {
    throw std::runtime_error("The priority queue is empty");
  }

  std::swap(items.front(), items.back());
  std::unique_ptr<int> _item_to_return = std::move(items.back());
  items.pop_back();
  _bubble_down();
